...
from deepopt.defaults import DELUQ_CONFIG, GP_CONFIG, NNENSEMBLE_CONFIG, Defaults
...
def test_config_settings_json_loading_currently_raises_type_error(tmp_path):
    ...
    with pytest.raises(TypeError, match="JSON object must be str"):
        ConfigSettings("delUQ", config_file=str(config_file))
...
def test_defaults_public_values_regression():
    ...
    assert Defaults.num_restarts_low == 5
    assert Defaults.num_restarts_high == 15
    assert Defaults.raw_samples_low == 512
    assert Defaults.raw_samples_high == 5000
    assert Defaults.n_fantasies == 128
